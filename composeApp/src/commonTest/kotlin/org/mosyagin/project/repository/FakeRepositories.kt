...
class FakeSyncRepository : SyncRepository {
    ...
    override suspend fun markSynced(ids: List<Long>) {
        queue.value = queue.value.map { if (ids.contains(it.id)) it.copy(synced = 1L) else it }
    }
...
class FakeAuthRepository : AuthRepository {
    ...
    private val _isEncryptionReady = MutableStateFlow(true)
    ...
    override suspend fun createRecoveryBackup(pin: String): Boolean = true
    override suspend fun restoreFromBackup(pin: String): Boolean = true
    override suspend fun isBackupAvailable(): Boolean = false
    override suspend fun generateNewKeys(pinForBackup: String?) {}
    
    fun setUser(user: UserInfo?) { _currentUser.value = user }
}
...
class FakeProjectRepository : ProjectRepository {
    ...
    override suspend fun deleteProject(id: String) { projects.value = projects.value.filter { it.id != id } }
    override suspend fun updateProject(id: String, name: String, director: String) {
        projects.value = projects.value.map { if (it.id == id) it.copy(name = name, director = director) else it }
    }
    override suspend fun markProjectAsRemote(id: String) {
        projects.value = projects.value.map { if (it.id == id) it.copy(isRemote = 1L) else it }
    }
}
...
class FakeScriptRepository : ScriptRepository {
    ...
    override fun getScriptFileById(id: String): Flow<ScriptFile?> = flowOf(scripts.value.find { it.id == id })
    override suspend fun saveParsedScript(projectId: String, seriesNumber: Int, filePath: String, fullText: String, createdAt: Long) {
        val newId = generateUUID()
        scripts.value += ScriptFile(newId, projectId, seriesNumber.toLong(), "Серия $seriesNumber", filePath, createdAt, null, "White", null, 0L)
    }
    override suspend fun deleteScriptFile(fileId: String) { scripts.value = scripts.value.filter { it.id != fileId } }
    override suspend fun updateScriptTitle(fileId: String, newTitle: String) {
        scripts.value = scripts.value.map { if (it.id == fileId) it.copy(title = newTitle) else it }
    }
}
...
class FakeSceneRepository : SceneRepository {
    ...
    override fun getSceneById(sceneId: String, scriptFileId: String): Flow<GetSceneById?> = flowOf(null)
    ...
    override fun getLatestScenesForProject(projectId: String): Flow<List<GetLatestScenesForProject>> = flowOf(emptyList())
    ...
    override suspend fun addPropFull(sceneUserDataId: String, name: String, anchor: String, status: String, category: String, quantity: Int, actorId: String?, note: String?, isCrossCutting: Boolean, groupId: String?): String = ""
...
class FakeShiftRepository : ShiftRepository {
    ...
    private val links = mutableListOf<Triple<String, String, Long>>()
